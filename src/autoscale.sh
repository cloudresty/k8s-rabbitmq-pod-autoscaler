...
function getCurrentPods() {
  ...
  for i in $(seq 5); do
    ...
    current=$(kubectl describe deployment $deployment -n $namespace | grep desired | awk '{print $2}' | head -n1)
...
while true; do
  ...
  for autoscaler in "${autoscalingArr[@]}"; do
    ...
    queueMessagesJson=$(curl -s -S --retry 3 --retry-delay 3 -u $RABBIT_USER:$RABBIT_PASS $RABBIT_HOST/api/queues/$vhost/$queueName)
